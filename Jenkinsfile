pipeline{
    ...
    stages {
        ...
        stage("CD") {
            steps {
                withCredentials([usernamePassword(credentialsId: 'nexus_credentials',usernameVariable: 'NEXUS_USERNAME',passwordVariable: 'NEXUS_PASS')]){
                  ah 'kubectl create secret docker-registry regcred   --namespace=dev   --docker-server=10.99.146.53:5000   --docker-username=${NEXUS_USERNAME}   --docker-password=${NEXUS_PASS}   --docker-email=admin@example.org'
                  sh 'docker login 10.99.146.53:5000 -u ${NEXUS_USERNAME}  -p ${NEXUS_PASS}'
